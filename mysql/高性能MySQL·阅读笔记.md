...
MySQL 会解析查询，并创建内部数据结构（解析树），然后对其进行各种优化，
...
MySQL 服务器层不管理事务，事务时由下层的存储引擎实现的，所以在同一个事务中，使用多种存储引擎是不可靠的。
...
下面看一下在 REPEATABLE READ 隔离级别下，MVCC 是如何发光发热的：

+ SELECT

    + InnoDB 会根据以下两个条件检查每行记录：

    + a. InnoDB 只查找版本早于当前事务版本的数据行（也就是行的系统版本号小于或等于事务的系统版本号），
...
其他两个隔离级别都和 MVCC 不兼容，因为 READ UNCOMMITTED 总是读取最新的数据行，
而不是符合当前事务版本的数据行。而 SERIALIZABLE 则会对所有读取的行都加锁。

### 1.5 MySQL的存储引擎

本节概要地描述了 MySQL 的存储引擎。

### 1.6 MySQL时间线（Timeline）

本节主要讲述了 MySQL 版本的变迁历史。

### 1.7 MySQL的开发模式

本节主要讲述了 MySQL 的开发过程和发布模型。
